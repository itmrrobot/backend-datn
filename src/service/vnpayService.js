      ...
      if (checkAmount) {
        if (paymentStatus == "0") {
          ...
          if (rspCode == "00") {
            ...
            res.redirect("https://itmrrobot.github.io/Frontend-datn/vnpay/payment/success");
          } else {
            //that bai
            //paymentStatus = '2'
            // Ở đây cập nhật trạng thái giao dịch thanh toán thất bại vào CSDL của bạn

            res.redirect("https://itmrrobot.github.io/Frontend-datn/vnpay/payment/cancel");
          }
        } else {
          res
            .status(200)
...
const getPaymentSuccess = async (res) => {
  await orderService.createNewOrder({ ...order, status: 2 });
  cartIds?.forEach(async (id) => {
    await cartService.deleteCard(id);
  });
...
module.exports = { createPayment, getPaymentSuccess, getVnpayIPN };
